  ...
  module Hooks
    class HelpdeskHooks < Redmine::Hook::Listener
      def after_plugins_loaded(_context = {})
        ...
        Journal.send(:include, RedmineHelpdesk::Patches::JournalPatch) unless Journal.included_modules.include? RedmineHelpdesk::Patches::JournalPatch
        # Add module to MailHandler class
        MailHandler.send(:include, RedmineHelpdesk::Patches::MailHandlerPatch) unless Journal.included_modules.include? RedmineHelpdesk::Patches::MailHandlerPatch
        # Add module to Mailer class
        Mailer.send(:include, RedmineHelpdesk::Patches::MailerPatch) unless Journal.included_modules.include? RedmineHelpdesk::Patches::MailerPatch
      ...
      def view_issues_edit_notes_bottom(context={})
        ...
        context = ActionView::Base.with_empty_template_cache.new( 
          lookup_context, 
          { 
            :email => owner_email, 
        ...
        renderer.render(context, { partial: 'issue_edit' })
      ...
      def view_issues_history_journal_bottom(context={})
        ...
        lookup_context = ActionView::LookupContext.new([File.dirname(__FILE__) + '/../../../app/views/'])
        context = ActionView::Base.with_empty_template_cache.new( 
          lookup_context, {:email => owner_email}, nil
        )
        renderer = ActionView::Renderer.new(lookup_context)
        renderer.render(context, { partial: 'issue_history' })
        # action_view = ActionView::Base.new(File.dirname(__FILE__) + '/../app/views/')
