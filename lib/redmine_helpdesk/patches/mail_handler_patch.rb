    ...
    module MailHandlerPatch
      ...
      module InstanceMethods
        ...
        def dispatch_to_default_with_helpdesk
          issue = receive_issue
          roles = if issue.author.is_a?(AnonymousUser)#class == AnonymousUser
            #Role.anonymous #where(builtin: issue.author.id)
            issue.author.builtin_role
          else
          ...
          if issue.author.is_a?(AnonymousUser) || roles.any? {|role| role.allowed_to?(:treat_user_as_supportclient) } #type.eql?("AnonymousUser")
...
# MailHandler.include RedmineHelpdesk::Patches::MailHandlerPatch
